...
- **Tasks Completed:**
  ...
  - [x] Sized the always-on-top pin overlay to cover the target window instead of a hardcoded 32x32 size, adjusted to start 8px higher from the top (`y = rect.top - 8`, `height = rect.height() + 8`).
  ...
  - [x] Integrated conditional drawing of the system accent border with rounded corners (8px radius) in `update_pinned_overlay_graphics` only when the pinned window is focused.
...
- **Solutions Applied:**
  - Resized the pin overlay to match the window bounds, extended 8px taller from the top (`rect.width()` and `rect.height() + 8`), and positioned the pin icon dynamically in the top-right corner of the canvas using `blit_pixmap`.
  ...
  - Updated `draw_border` in `src/ui/draw.rs` to build a rounded path using `pb.quad_to` and draw it with an 8px corner radius.
