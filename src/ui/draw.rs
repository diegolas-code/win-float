...
pub fn draw_hud(
    ...
    // Track background
    let track_rect =
        SkiaRect::from_ltrb(slider_x, slider_y, slider_x + slider_w, slider_y + slider_h)
    ...
    if percentage > 0 {
        ...
        let fill_rect =
            SkiaRect::from_ltrb(slider_x, slider_y, slider_x + fill_w, slider_y + slider_h)
...
pub fn draw_pin(canvas: &mut Canvas, _accent_color: Color) -> Result<(), String> {
    ...
    if let Some(ref path) = body_path {
        let t = Transform::from_scale(0.85, 1.15)
            .post_translate(cx * (1.0 - 0.85), (cy + 1.0) * (1.0 - 1.15));
...
pub fn blit_pixmap(dest: &mut Pixmap, src: &Pixmap, dx: u32, dy: u32) {
    ...
    for y in 0..src_h {
        ...
        for x in 0..src_w {
            ...
            dest_data[dest_idx] = (src_data[src_idx] as f32 + dest_data[dest_idx] as f32 * factor)
                .clamp(0.0, 255.0)
                .round() as u8;
            dest_data[dest_idx + 1] = (src_data[src_idx + 1] as f32
                + dest_data[dest_idx + 1] as f32 * factor)
                .clamp(0.0, 255.0)
                .round() as u8;
            dest_data[dest_idx + 2] = (src_data[src_idx + 2] as f32
                + dest_data[dest_idx + 2] as f32 * factor)
                .clamp(0.0, 255.0)
                .round() as u8;
            dest_data[dest_idx + 3] = (src_data[src_idx + 3] as f32
                + dest_data[dest_idx + 3] as f32 * factor)
                .clamp(0.0, 255.0)
                .round() as u8;
        }
    }
