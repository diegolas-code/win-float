        ...
        match id {
            HOTKEY_TOPMOST_ID => {
                ...
                if new_state {
                    ...
                    let overlay = self.overlay_manager.create_overlay(active, rect.left, rect.top - 10, width, height)?;
    ...
    pub fn handle_window_moved(&mut self, target_hwnd: HWND) -> Result<(), String> {
        ...
        if let Some(overlay) = overlay_hwnd {
            ...
            let (ox, oy, ow, oh) = if is_modal {
                ...
                (rect.left, rect.top - 10, rect.width(), rect.height() + 10)
            };
    ...
    fn test_controller_topmost_toggle() {
        ...
        *controller.window_manager.active_window.lock().unwrap() = target;
        ...
        
        {
            ...
            assert_eq!(oy, -10, "overlay y should start at top - 10");
            assert_eq!(oh, 610, "overlay height should be rect.height() + 10");
        }
